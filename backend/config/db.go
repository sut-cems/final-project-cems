...
func setupActivityReports() {
    ...
    if len(students) > 0 {
        sampleReports := []entity.ActivityReport{
            {
                UserID:      students[0].ID,
                Type:        "transcript",
                FileURL:     "/reports/student_transcript_" + students[0].StudentID + ".pdf",
                GeneratedAt: time.Now().AddDate(0, 0, -5),
            },
            {
                UserID:      students[0].ID,
                Type:        "activity_summary",
                FileURL:     "/reports/activity_summary_" + students[0].StudentID + ".pdf",
                GeneratedAt: time.Now().AddDate(0, 0, -2),
            },
            {
                UserID:      students[1].ID,
                Type:        "transcript",
                FileURL:     "/reports/student_transcript_" + students[1].StudentID + ".pdf",
                GeneratedAt: time.Now().AddDate(0, 0, -3),
            },
        }

        if adminUser.ID != 0 {
            adminReports := []entity.ActivityReport{
                {
                    UserID:      adminUser.ID,
                    Type:        "system_report",
                    FileURL:     "/reports/system_activity_report_" + time.Now().Format("2006-01") + ".pdf",
                    GeneratedAt: time.Now().AddDate(0, 0, -1),
                },
                {
                    UserID:      adminUser.ID,
                    Type:        "club_statistics",
                    FileURL:     "/reports/club_statistics_" + time.Now().Format("2006-01") + ".pdf",
                    GeneratedAt: time.Now().AddDate(0, 0, -7),
                },
            }
            sampleReports = append(sampleReports, adminReports...)
        }

        for _, report := range sampleReports {
            var existing entity.ActivityReport
            result := db.Where("user_id = ? AND type = ? AND file_url = ?", 
                report.UserID, report.Type, report.FileURL).First(&existing)
            if result.Error != nil {
                db.Create(&report)
            }
        }
    ...
    fmt.Println("Activity reports setup completed")
}